...
- **Omarchy Launch Feedback & Window Lifecycle Fix:**
  ...
  - Added `focus_hyprland_window("OmoModular")`: querying Hyprland clients via `hyprctl clients -j` and dispatching `hl.dsp.focus` (with `focuswindow` fallback) to bring existing instances to the foreground immediately rather than spawning duplicate detached windows on background workspaces.
  - Asynchronous uvicorn startup coordination: replaced synchronous browser launch prior to `uvicorn.run()` with `asyncio.gather(server.serve(), _opener())` and `wait_for_port()`. Chromium now launches only after port 8796 is verified accepting connections, preventing connection refused race conditions and launch latency spikes.
  - Stale Chromium singleton lock eviction: added `clean_stale_singleton(_PROFILE_DIR)` to prune dead `SingletonLock` / `SingletonSocket` symlinks before launching Chromium, eliminating browser startup delays caused by crashed or killed sessions.
  - Desktop Entry Wayland alignment: added `StartupNotify=false` to `omomodular.desktop` to prevent desktop managers and `gtk-launch` from tracking unresolvable Wayland WMClass notifications for Chromium `--app` surfaces.
- **Expanded Percussion, Sequencers & Advanced Dynamics Suite (8 New Modules, 51 Total):**
  - **Open Sample Drum (`sample_player`):** 5 classic vintage drum machine kits (TR-909, TR-707, LinnDrum, Oberheim DMX, Roland CR-78) across 25 synthesized PCM voices. Features SP-1200 style 12-bit decimation/crunch, pitch transposition (-12 to +12 semitones), sample decay, start point slice offset, resonant lowpass tone filter, drag-and-drop external `.wav` file loading, trigger input, audio output, and End-of-Cycle (`eoc`) trigger output.
  - **Macro Percussion Voice (`macro_percussion`):** Procedural drum synthesizer inspired by Mutable Instruments Plaits / Noise Engineering Basimilus. 6 specialized synthesis models (Sub FM Bass Drum, Metallic Snappy Snare, Folded Metallic Hat, Multi-burst Clap, Modular FM Laser Zap, and Wooden Rim/Block) with non-linear wavefolding, harmonic drive, and accent punch.
  - **Amen Break Slicer (`amen_slicer`):** 16-step transient chopper breakbeat workstation locked to Master BPM. Features forward and reverse playback per slice, jungle stutter roll re-triggering, lo-fi grit drive, 24dB resonant filter sweep, drag-and-drop breakbeat audio loading, and step trigger / gate outputs.
  - **Euclidean Poly-Rhythm Sequencer (`quad_euclid`):** 4-track Bjorklund algorithmic polyrhythm engine with 4 independent trigger outputs (`trig1` - `trig4`). Independent step counts (1-16), active fill pulses (0-16), and rotational beat offsets (0-15) per track, rendered on concentric glowing LED step rings. Direct zero-latency cable dispatch to drum modules.
  - **Stochastic Random CV & Melody Generator (`stochastic_vault`):** Mutable Instruments Marbles inspired generative melody and rhythmic gate engine. Features Déjà Vu looping memory lock, musical scale quantization (Pentatonic, Dorian, Phrygian, Hirajoshi, Natural Minor, Major, Chromatic), 1V/oct pitch CV out, dual rhythmic gate outputs, and smooth slewed CV.
  - **Dual Morphing Wavetable VCO (`wavetable_dual`):** Dual periodic wavetable oscillator using band-limited Fourier harmonic series (PPG Bell, Vocal Formant, Metallic, Harsh Digital Saw, Organ Glass). Features continuous wavetable position scanning, cross-FM synthesis, sub-octave bass foundation, and dual unipolar CV modulation inputs.
  - **Sidechain Ducking VCA (`sidechain_vca`):** Dynamic ducking amplifier and envelope follower. Attenuates basslines, pads, and drone synths whenever kicks or trigger pulses arrive to prevent low-end frequency collisions and create rhythmic pumping. Real-time gain reduction (-dB) meter, audio peak / pulse detection modes, and audition tap.
  - **16-Step TR Drum Matrix (`tr_matrix_seq`):** Classic 4-track (BD, SD, CH, OH) clickable 16-step drum machine sequencer with 4-beat color grouping, swing delay, accent boost, real-time step playhead tracking, random pattern generation, and 5 discrete trigger/accent outputs.
- **Online & Offline Sample Archive Drawer (Dual Media Drawer):**
  - Upgraded the left sliding drawer into a unified dual-tab sound repository: `[♪ SAMPLES]` and `[♫ MIDI FILES]` accessible via header `[M] ARCHIVES` or keyboard shortcut `[M]`.
  - **12 Bundled Offline Starter Samples:** Embedded high-quality 16-bit 44.1kHz `.wav` percussion samples across 5 categories: Kicks (808 Sub Kick, 909 Punch Kick, Linn Retro Kick), Snares (909 Crisp Snare, 707 Rim Snare, 808 Handclap), Hi-Hats (909 Open Hat, 808 Closed Hat), Percussion (DMX Cowbell, Modular Laser Zap, Wooden Clave), and Breakbeats (165 BPM Amen Breakbeat).
  - **Freesound CC0 Creative Commons Search Integration:** Hybrid online search query engine querying Freesound REST API v2 for royalty-free CC0 drum samples. Optional user API key configuration modal persisted to `~/.config/omomodular/freesound_key.txt` with zero-config offline fallbacks.
  - **Instant Web Audio Audition Playback:** Sample cards feature zero-latency `[▶ PLAY]` / `[■ STOP]` audition previewing directly through the DSP engine before committing to the rack.
  - **Smart Drag-and-Drop Sample Loading:** Draggable sample cards (`text/sample-payload`). Dropping onto an empty Eurorack rail slot automatically spawns an `Open Sample Drum` (`sample_player`) module (or `Amen Break Slicer` for breakbeats) with that sample loaded into memory. Dropping onto an existing `sample_player` or `amen_slicer` faceplate updates its sound buffer and live waveform display immediately.
- **Factory Presets:**
  - Added `★ TR-Matrix Drums & Ducking Sidechain Bass` (16-step TR-style clickable drum sequencer driving 909 kick, snare, and metallic hats, with a Sidechain Ducking VCA pumping a resonant sub-bass drone out of the kick's path).
  - Added `★ Euclidean Breakbeat & Modular Sample Jam` (Quad Euclidean sequencer driving 909 kick, 707 snare, macro percussion, and stochastic wavetable lead through delay/reverb into master console).
  - Added `★ Jungle Amen Break & West-Coast Acid` (165 BPM Amen break slicer in jungle stutter mode with macro sub kick, 303 acid line, tape warmer, and spring tank).
